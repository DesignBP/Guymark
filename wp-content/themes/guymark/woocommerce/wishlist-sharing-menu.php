...
if ($wishlist_items && count($wishlist_items)) :
	...
	<?php if (strstr($wishlist->get_wishlist_sharing(), 'Private') === false && ($e_email | $e_facebook | $e_twitter | $e_pinterest)) : ?>
		<ul class="wl-share-links">
			<li><?php // _e('Share', 'wc_wishlist'); ?></li>
		<?php if ($e_email) : ?>
				<li class="wl-email"><a class="wl-email-button" href="#share-via-email-<?php echo $wishlist->id; ?>"><?php _e('Email', 'wc_wishlist'); ?></a></li>
		<?php endif; ?>
			<?php if ($e_facebook) : ?>
				<li class="wl-facebook">
                    ...
                    // echo $url;
                    ?>
					<a onClick="window.open('http://www.facebook.com/sharer.php?s=100&amp;p[title]=<?php echo $title; ?>&amp;p[summary]=<?php echo $summary; ?>&amp;p[url]=<?php echo $url; ?>&amp;p[images][0]=<?php echo $image; ?>', 'sharer', 'toolbar=0,status=0,width=548,height=325');" href="javascript: void(0)"><?php _e('Facebook', 'wc_wishlist'); ?></a>
				</li>
			<?php endif; ?>
			<?php if ($e_twitter) : ?>
				<li class="wl-twitter"><a target="_blank" href="<?php echo $twitter_url; ?>"><?php _e('Twitter', 'wc_wishlist'); ?></a></li>
			<?php endif; ?>
			<?php if ($e_pinterest && $pinterest_img_url) : ?>
				<li class="wl-pinterest">
					<a href="http://pinterest.com/pin/create/button/?url=<?php $wishlist->the_url_view(true); ?>&media=<?php echo $pinterest_img_url; ?>&amp;description=<?php $wishlist->the_title(); ?>" class="hide-text" target="_blank"><?php _e('Pin It on Pinterest!', 'wc_wishlist'); ?></a>
				</li>
			<?php endif; ?>
		</ul>
	<?php endif; ?>
