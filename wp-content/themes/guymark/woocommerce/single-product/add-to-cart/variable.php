...
foreach($available_variations as $variation){
	...
	$sku_lookup[$label] = $variation['sku'];
}
?>
...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo $post->ID; ?>" data-product_variations="<?php echo esc_attr( json_encode( $available_variations ) ) ?>">
	<table class="variations" cellspacing="0">
		<tbody>
			<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
				<tr>
					<td class="label"><label for="<?php echo sanitize_title($name); ?>"><?php echo db_make_readable($woocommerce->attribute_label( $name )); ?></label></td>
							...
							if ( is_array( $options ) ) {
								...
								if ( taxonomy_exists( $name ) ) {
									...
									foreach ( $options as $option ) {
										...
										echo '<option value="' . esc_attr( sanitize_title( $option ) ) . '" ' . selected( sanitize_title( $selected_value ), sanitize_title( $option ), false ) . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', $option ) ) . ' - ' . $sku_lookup[sanitize_title( $option )] . '</option>';
									}
	...
	<?php do_action('woocommerce_before_add_to_cart_button'); ?>
		...
		<div class="single_variation_wrap">
			...
			<div class="variations_button product buttons">
				...
				<?php if( db_is_available_for_purchase() && ($_GET['display'] != "information")) : ?>
