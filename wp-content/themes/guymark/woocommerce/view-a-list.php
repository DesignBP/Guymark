	...
	<div id="wl-wrapper" class="product woocommerce">
		...
		<?php if ($wishlist_items && count($wishlist_items)) : ?>
			<?php if (isset($_GET['preview']) && $_GET['preview']) : ?>
				<div class="woocommerce-info woocommerce_info"><a href="<?php echo $wishlist->the_url_edit(); ?>" class="button"><?php _e('Return to your view', 'wc_wishlist'); ?></a><br /> <?php _e('This is how other people will see your inventory list.', 'wc_wishlist'); ?></div>
			<?php endif; ?>

			<div class="wl-intro">
				<h3 class="entry-title"><?php $wishlist->the_title(); ?></h3>
			...
			<div class="wl-row wl-clear">
				<form method="GET" action="<?php $wishlist->the_url_view(); ?>">
					...
					<table width="100%" cellpadding="0" cellspacing="0" class="wl-actions-table wl-left">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<?php wp_dropdown_categories(array('taxonomy' => 'product_cat', 'class' => 'wl-sel', 'name' => 'wlitemcat', 'show_option_all' => __('All', 'wc_wishlist'), 'selected' => $wlitemcat, 'hierarchical' => true, 'include' => array_keys($wishlist_item_categories))); ?>
								</td>
								<td>
									<input type="submit" class="button small wl-but" value="<?php _e('Go', 'wc_wishlist'); ?>" />
								</td>
				...
				<tbody>
					...
					if (sizeof($wishlist_items) > 0) :
							...
							if ($_product->exists() && $item['quantity'] > 0) :
								...
								<tr class="<?php echo esc_attr(apply_filters('woocommerce_cart_item_class', 'cart_item cart_table_item', $item, $wishlist_item_key)); ?>">
									...
									<td class="product-name">
										...
										if (WC_Wishlist_Compatibility::is_wc_version_gte_2_1()) {
											if (!$_product->is_visible()) {
												...
												echo apply_filters('woocommerce_cart_item_name', sprintf('<a href="%s">%s</a>', ( is_array($item['variation']) ? add_query_arg($item['variation'], $_product->get_permalink()) : $_product->get_permalink()), $_product->get_title()), $item, $wishlist_item_key);
											}
										} else {
									...
									<td class="product-price">
										<?php
										if (WC_Wishlist_Compatibility::is_wc_version_gte_2_1()) {
											echo apply_filters('woocommerce_cart_item_price', WC()->cart->get_product_price($_product), $item, $wishlist_item_key);
										} else {
									...
									<td class="product-purchase">
										<?php if ($_product->is_in_stock() && apply_filters('woocommerce_wishlist_user_can_purcahse', true, $_product)) : ?>
											<a href="<?php echo woocommerce_wishlist_url_item_add_to_cart($wishlist->id, $wishlist_item_key, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false ); ?>" class="button"><?php _e('Add to basket', 'wc_wishlist'); ?></a>
										<?php endif; ?>
						...
						<tr><!-- the tds need to be individual for woothemes responsive mode, since they target the first td specifically and hide it. WIth colspan this causes issues once in responsive mode -->
							...
							<td class="product-purchase"><a href="<?php echo woocommerce_wishlist_url_add_all_to_cart($wishlist->id, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false ); ?>" class="button alt wl-add-all"><?php _e('Add All To Basket', 'wc_wishlist'); ?></a></td>
						</tr>
					<?php endif; ?>
				</tbody>
			</table>
		...
		<?php else : ?>
			...
			<div class="woocommerce-info woocommerce_info"> <?php _e('This list currently contains no items.', 'wc_wishlist'); ?> <a href="<?php echo WC_Wishlists_Pages::get_url_for('find-a-list'); ?>"><?php _e('Back to find a list', 'wc_wishlist'); ?></a></div>
			<?php endif; ?>
