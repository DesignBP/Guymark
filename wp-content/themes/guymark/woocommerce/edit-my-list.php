...
<div id="wl-wrapper" class="product woocommerce"> <!-- product class so woocommerce stuff gets applied in tabs -->
	...
	<div class="wl-intro">
		...
		<?php if ($sharing == 'Public' || $sharing == 'Shared') : ?>
			<div class="wl-share-url"><strong><?php _e('Inventory URL:', 'wc_wishlist'); ?> </strong><?php echo $wishlist->the_url_view($sharing == 'Shared'); ?></div>
		<?php endif; ?>
		<?php if ($sharing == 'Public' || $sharing == 'Shared') : ?>
		...
		<p><a class="wlconfirm" data-message="<?php _e('Are you sure you want to delete this list?', 'wc_wishlist'); ?>" href="<?php $wishlist->the_url_delete(); ?>"><?php _e('Delete list', 'wc_wishlist'); ?></a>
		...
		<div class="wl-panel panel" id="tab-wl-items">
								...
								<tr>
									<td>
										<select class="wl-sel move-list-sel" name="wlupdateaction" id="wleditaction1">
											...
											<option value="add-to-cart"><?php _e('Add to Basket', 'wc_wishlist'); ?></option>
								...
								if ($_product->exists() && $item['quantity'] > 0) {
									...
									<tr class="cart_table_item">
										<td class="check-column" >
											<?php if( db_is_available_for_purchase($_product->id) ) : ?>
											<?php endif; ?>
											<input type="checkbox" name="wlitem[]" value="<?php echo $wishlist_item_key; ?>" />
										</td>
										<td class="product-remove">
											<a href="<?php echo woocommerce_wishlist_url_item_remove($wishlist->id, $wishlist_item_key); ?>" class="remove wlconfirm" title="<?php _e('Remove this item from your inventory', 'wc_wishlist'); ?>" data-message="<?php esc_attr(_e('Are you sure you would like to remove this item from your list?', 'wc_wishlist')); ?>">&times;</a>
										</td>
										...
										<td class="product-purchase">
											<?php if( db_is_available_for_purchase($_product->id) ) : ?>
												<?php if ($_product->is_in_stock() && apply_filters('woocommerce_wishlist_user_can_purcahse', true, $_product)) : ?>
													<a href="<?php echo woocommerce_wishlist_url_item_add_to_cart($wishlist->id, $wishlist_item_key, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false, 'edit'); ?>" class="button alt"><?php _e('Add to Basket', 'wc_wishlist'); ?></a>
												<?php endif; ?>
											<?php endif; ?>
										</td>
									</tr>
			...
			<?php else : ?>
				<p>
					...
					<?php _e('You do not have anything in this list.', 'wc_wishlist'); ?> <a href="<?php echo $shop_url; ?>"><?php _e('Visit shop', 'wc_wishlist'); ?></a>.
				</p>
			<?php endif; ?>
...
<strong>What kind of list is this?<abbr class="required" title="required">*</abbr></strong>
            <table class="wl-rad-table">
                    <tr>
                            ...
                            <td><label for="rad_wishlist">Inventory <span class="wl-small">- Allows you to add products to a list for later.</span></label></td>
                    </tr>
                    <tr>
					...
					<p class="form-row form-row-wide" style="margin-top: 0px;">
