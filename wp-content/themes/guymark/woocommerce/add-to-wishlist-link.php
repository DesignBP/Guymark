...
<div id="wl-wrapper" class="product buttons woocommerce wl-button-wrap wl-row wl-clear <?php echo $product->is_type('variable') ? 'hide' : ''; ?>" >
	<?php if (woocommerce_wishlists_get_wishlists_for_product($product->id)) : ?>
		<div class="wl-already-in">
			<?php _e('This item is already in one of your Inventory lists', 'wc_wishlist'); ?>
			<ul>
	...
	<a href="" data-listid="<?php echo $add_to_wishlist_args['single_id']; ?>" class="button inventory<?php echo implode(' ', $add_to_wishlist_args['btn_class']); ?>">
		<?php echo apply_filters('woocommerce_wishlist_add_to_wishlist_text', WC_Wishlists_Settings::get_setting('wc_wishlist_button_text', 'Add to inventory'), $product->product_type); ?>
	</a>
</div>
