...
function show_product($product){
	...
	$wishlisturl = $carturl . "&add-to-wishlist-itemid=" . $post->ID;
	setup_postdata($post);
	$productid = $post->ID;
	
	$_name = get_the_title($productid);
	$_kf2  = get_field("key_features", $productid);
	$_manufacturer = get_field("manufacturer", $productid);	
	$_sku = $product->get_sku();
	$_kf  = "";
	$_kf3  = 0;
	foreach( $_kf2 as $field=>$feature ){
		$_kf .= "<li>" . $feature['feature'] . "</li>" . "\n";
		$_kf3++;
	}
	
	$image = "/wp-content/plugins/woocommerce/assets/images/placeholder.png";
	if (has_post_thumbnail($productid)){
		$image_title 	= esc_attr($_name);
	...
	$hide_productid = (($_sku == "") ? "displaynone" : "");
	$hide_manufacturer = (($_manufacturer == "") ? "displaynone" : "");
	$hide_features = (($_kf3 == 0) ? "displaynone" : "");
	
	$additionalbuttons = "";
	if(isset($_GET['filter']) || strtolower(get_the_title()) == "shop"){
		$additionalbuttons =<<<ENDL2
			<a class="addtobasket" href="{$carturl}">Add to basket</a>
			<a class="addtoinventory" href="{$wishlisturl}">Add to inventory</a>	
ENDL2;
	}
...
echo <<<ENDL
	<div class="product">
		...
		<div class="right">
			<h2>{$_name}</h2>
			<div class="keyval {$hide_productid}">
				<div class="key">Product Code:</div>
				<div class="value">{$_sku}</div>
				<div class="end"></div>
			</div>
			<div class="keyval {$hide_manufacturer}">
				<div class="key">Manufacturer:</div>
				<div class="value">{$_manufacturer}</div>
				<div class="end"></div>
			</div>
			<div class="featurebox {$hide_features}">
				<div>Key features:</div>
				<ul>
					{$_kf}
				</ul>
			</div>
			<a class="findoutmore" href="{$link}">Find out more</a>
			{$additionalbuttons}
...
if (isset($_GET['redirect_to']) && isset($_GET['wlid'])){
