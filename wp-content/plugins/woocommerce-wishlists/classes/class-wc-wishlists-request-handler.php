...
class WC_Wishlists_Request_Handler {
	public static function process_request() {
		...
		if (isset($_REQUEST['wlaction'])) {
			...
			if($action == "default-list"){
				WC_Wishlists_Plugin::verify_nonce($action, "_POST", "");
			}
			else {
				WC_Wishlists_Plugin::verify_nonce($action);
			}
	...
	private static function default_list() {
		global $woocommerce;
		
		$post_id = $_REQUEST['wlno'];
		...
		delete_user_meta(get_current_user_id(), "inventory_default");
		if(update_user_meta(get_current_user_id(), "inventory_default", $post_id)){
			$wishlist_name = get_the_title($post_id);
			WC_Wishlist_Compatibility::wc_add_notice(__('Your primary inventory list is now set to \'' . $wishlist_name . "'.", 'wc_wishlist') );
			$result = 1;
			return true;
		}
		else {
			WC_Wishlist_Compatibility::wc_add_notice(__('Error setting default wishlist', 'wc_wishlist') );
			return false;
		}
	}
	...
	private static function edit_lists() {
		...
		foreach ($listids as $id => $sharing) {
			...
			if ($wl_owner != $current_owner_key && !current_user_can('manage_woocommerce') && strlen($wl_owner) != 0) {
				wp_die(__('You can only update your own lists', 'wc_wishlist'));
			}
	...
	private static function add_to_cart($wishlist_id = false, $wishlist_item_key = false, $suppress_messages = false) {
		...
		if ($wishlist_item_key) {
			if (sizeof($wishlist_items > 0) && isset($wishlist_items[$wishlist_item_key])) {
				...
				$available_for_purchase = db_is_available_for_purchase( (int) $cart_item['product_id'] );
				$unable_to_add = false;
				
				if(!$available_for_purchase){
					$unable_to_add = true;
					WC_Wishlist_Compatibility::wc_add_notice(__('An item you have selected is not available for purchase.', 'wc_wishlist'), 'error');
					$result = 0;
				}
				else {
				
					if ((!$unable_to_add) && $woocommerce->cart->add_to_cart((int) $cart_item['product_id'], $cart_item['quantity'], $cart_item['variation_id'], $cart_item['variation'], $add_on_data)) {
